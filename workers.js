...
export default {
    async fetch(request, env, ctx) {
        ...
        // CFspider 公开模式：使用固定的公开 UUID，无需配置
        const PUBLIC_UUID = 'cfspider-public-00000000-0000';  // 公开 UUID 前缀
        const 管理员密码 = env.ADMIN || env.admin || env.PASSWORD || env.password || env.pswd || env.TOKEN || env.KEY || env.UUID || env.uuid || 'cfspider-public';
        const 加密秘钥 = env.KEY || 'cfspider-default-key';
        ...
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            // CFspider 首页 - 当访问根路径时显示 CFspider 界面
            const cfspiderPath = url.pathname.slice(1).toLowerCase();
            ...
            // CFspider API 路由
            if (cfspiderPath === 'api/pool') {
                return new Response(JSON.stringify(generateIPPool(request)), {
            ...
            if (cfspiderPath === 'api/config/new_ip' && request.method === 'POST') {
                ...
                // 这里只是返回当前状态，实际需要通过 Cloudflare Dashboard 或 wrangler.toml 设置
                const newIpEnabled = env.NEW_IP !== 'false' && env.NEW_IP !== '0';
            return new Response(JSON.stringify({
                    new_ip: newIpEnabled,
                    message: '请通过 Cloudflare Dashboard 或 wrangler.toml 设置 NEW_IP 环境变量'
                }), { 
                    headers: { 
                        'Content-Type': 'application/json',
                        'Access-Control-Allow-Origin': '*'
...
// ==================== CFspider 界面函数 ====================
globalThis.START_TIME = globalThis.START_TIME || Date.now();

function generateIPPool(request) {
    const colo = request.cf?.colo || 'UNKNOWN';
    const country = request.cf?.country || 'XX';
    const ipPool = [
        { ip: `${colo}.edge.cloudflare.com`, type: 'EDGE', status: 'ONLINE', latency: Math.floor(Math.random() * 50) + 10, region: country },
        { ip: `172.64.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`, type: 'ANYCAST', status: 'ONLINE', latency: Math.floor(Math.random() * 50) + 20, region: country },
        { ip: `172.67.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`, type: 'ANYCAST', status: 'ONLINE', latency: Math.floor(Math.random() * 50) + 30, region: country },
        { ip: `104.21.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`, type: 'ANYCAST', status: 'ONLINE', latency: Math.floor(Math.random() * 50) + 25, region: country },
        { ip: `162.159.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`, type: 'ANYCAST', status: 'ONLINE', latency: Math.floor(Math.random() * 50) + 35, region: country },
        { ip: `104.24.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`, type: 'ANYCAST', status: 'ONLINE', latency: Math.floor(Math.random() * 50) + 28, region: country }
    ];
...
<body>
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card"><div class="stat-label">${t.status}</div><div class="stat-value online">${t.online}</div></div>
